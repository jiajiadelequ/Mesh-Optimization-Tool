...
- `减面算法`：
  ...
  `重建网格（Remesh）` 会重新生成一张更规整的新网格，适合想强行整理拓扑的情况。
  `代理盒（Box Proxy）` 会按每个子零件的包围盒直接生成盒子代理，特别适合桌子、柜子、课桌这类结构分明的模型。
...
- `体素大小`：只在 `Remesh` 里使用，数值越小细节越多，数值越大模型越粗、越轻。
- `忽略小零件尺寸`：只在 `Box Proxy` 里使用，小于这个尺寸的零件会直接跳过。填 `0` 表示不过滤。
